...
void showData(ListBis L) {
	...
	while (B != nullptr) {
		...
		cout << "Kapasitas : " << B->info.kapasitas
		     << " (Terisi: " << terisi << ")\n";
		cout << "Harga     : Rp " << B->info.harga << endl;
		cout << "Penumpang : ";
...
void deletePenumpangByKTP(ListBis &L) {
	string noKTP;
	cout << "\nMasukkan No KTP penumpang yang akan dihapus: ";
	cin >> noKTP;
	...
	while (B != nullptr && !found) {
		...
		while (P != nullptr) {
			if (P->info.noKTP == noKTP) {
				...
				cout << "\n[SUCCESS] Penumpang " << P->info.nama
				     << " berhasil dihapus dari bis " << B->info.kodeBis << "!\n";
	...
	if (!found) {
		cout << "\n[ERROR] Penumpang dengan KTP " << noKTP << " tidak ditemukan dalam sistem!\n";
	}
}

// ============================================================
// FUNGSI VALIDASI KODE BIS (BARU)
// ============================================================
bool isKodeBisDuplicate(ListBis L, string kodeBis) {
	adrBis B = L.first;
	while (B != nullptr) {
		if (B->info.kodeBis == kodeBis) {
			return true;
		}
		B = B->next;
	}
...
void tampilkanKeuntungan(ListBis L) {
	...
	adrBis B = L.first;
	int totalKeuntunganKeseluruhan = 0;

	if (B == nullptr) {
		cout << "Belum ada data bis.\n";
		return;
	}

	while (B != nullptr) {
		int jumlahPenumpang = countPenumpang(B);
		int pendapatanBis = jumlahPenumpang * B->info.harga;
		int keuntunganBis = pendapatanBis * 10 / 100;
		totalKeuntunganKeseluruhan += keuntunganBis;

		cout << "Bis " << B->info.kodeBis << " (" << B->info.rute << ")\n";
		cout << "  Penumpang       : " << jumlahPenumpang << " orang\n";
		cout << "  Harga Tiket     : Rp " << B->info.harga << "\n";
		cout << "  Total Pendapatan: Rp " << pendapatanBis << "\n";
		cout << "  Keuntungan (10%): Rp " << keuntunganBis << "\n";
		cout << "----------------------------------------\n";

		B = B->next;
	}

	cout << "TOTAL KEUNTUNGAN KESELURUHAN: Rp " << totalKeuntunganKeseluruhan << "\n";
	cout << "========================================\n";
}
